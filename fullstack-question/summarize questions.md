# 1. Basic
## 1.1. OOP
1. Thuật ngữ OOP có nghĩa là gì?

2. Tại sao cần OOP?

3. Một số ngôn ngữ lập trình hướng đối tượng

4. Một số mô hình lập trình khác ngoài OOP là gì?

5. Lập trình có cấu trúc nghĩa là gì?

6. Các tính năng chính của OOP?

7. Lợi ích của sử dụng OOP?

8. Tại sao OOP lại phổ biến như vậy?

9. Lớp là gì?

10. Đối tượng là gì?

11. Tính đóng gói là gì?

12. Tính đa hình là gì?

13. Sự khác nhau giữa đa hình thời gian biên dịch và đa hình thời gian chạy?

14. C++ hỗ trợ đa hình thế nào?

15. Kế thừa là gì?

16. Trừu tượng là gì?

17. Một lớp chiếm bao nhiêu bộ nhớ?

18. Có phải lúc nào cũng cần tạo các đối tượng từ lớp không?

19. Constructor là gì

20. Các kiểu constructor trong C++?

21. Copy Constructor là gì?

22. Destructor là gì?

23. Có phải lớp và cấu trúc giống nhau?

24. Giải thích về tính kế thừa

25. Có bất kỳ hạn chế nào của kế thừa không?

26. Các kiểu kế thừa khác nhau?

27. Lớp con là gì?

28. Lớp cha là gì?

29. Interface là gì?

30. Đa hình tĩnh là gì

31. Đa hình động là gì

32. Sự khác biệt giữa nạp chồng và ghi đè?

33. Quá trình trừu tượng hóa dữ liệu được thực hiện như thế nào?

34. Lớp trừu tượng là gì?

35. Một lớp trừu tượng khác với một interface như thế nào?

36. Các chỉ định truy cập là gì và ý nghĩa của chúng?

37. Ngoại lệ là gì?

38. Xử lý ngoại lệ như thế nào?

39. Garbage collection trong OOP là gì?

40. Có thể chạy ứng dụng Java mà không triển khai các khái niệm OOP?
## 1.2. Cấu trúc dữ liệu
1. Bạn có thể giải thích sự khác nhau giữa cấu trúc tập tin và cấu trúc lưu trữ

2. Bạn có thể nói sự khác biệt giữa cấu trúc dữ liệu tuyến tính và phi tuyến tính?

3. Mảng là gì?

4. Mảng đa chiều là gì?

5. Danh sách liên kết là gì?

6. Danh sách liên kết là tuyến tính hay phi tuyến tính?

7. Danh sách liên kết có hiệu quả hơn mảng?

8. Giải thích trường hợp nào sử dụng danh sách liên kết và trường hợp nào sử dụng

9. Danh sách liên kết đôi (DDL) là gì?

10. Ngăn xếp là gì?

11. Hàng đợi là gì?

12. Ngăn xếp và hàng đợi khác nhau thế nào?

13. Giải thích quy trình đằng sau việc lưu trữ một biến trong bộ nhớ?

14. Triển khai hàng đợi bằng ngăn xếp

15. Triển khai ngăn xếp bằng hàng đợi

16. Hashmap trong Java là gì?

17. Yêu cầu đối với một đối tượng được sử dụng làm khóa hoặc giá trị trong HashMap là gì?

18. Xử lý xung đột trong HashMap với Java?

19. Độ phức tạp thời gian của thao tác get() và put() trong HashMap

20. Cơ sở dữ liệu nào được dùng cho triển khai bộ đệm LRU?

21. Hàng đợi ưu tiên là gì?

22. Có thể lưu trữ một khóa trùng lặp trong HashMap không?

23. Cấu trúc dữ liệu cây là gì?

24. Cây nhị phân là gì

25. Số lượng nút tối đa của cây nhị phân có chiều dài k

26. Viết hàm đệ quy tính chiều cao cây nhị phân trong Java

27. Viết hàm Java đếm số lượng nút trong cây

28. Duyệt cây là gì?

29. Cây tìm kiếm nhị phân là gì?

30. Cây AVL là gì?

31. In chế độ xem bên trái của bất kỳ cây nhị phân nào

32. Cấu trúc dữ liệu đồ thị là gì?

33. Ứng dụng của đồ thị?

34. Làm thế nào để biểu diễn đồ thị?

35. Sự khác biệt giữa cấu trúc dữ liệu cây và đồ thị

36. Sự khác biệt giữa BFS (Breadth First Search) và DFS (Delth First Search)

37. Khi nào sử dụng DFS tốt hơn BFS?

38. Sắp xếp tô pô trong đồ thị là gì?

39. Cho một bản đồ lưới 2D, m x n trong đó giá trị "1" đại diện cho đất và "0" đại diện cho nước, trả về số lượng các hòn đảo (được bao quanh bởi nước và được hình thành bằng cách kết nối các vùng đất liền kề theo 2 hướng - theo chiều dọc hoặc chiều ngang). Giả sử rằng các trường hợp ranh giới - tức là tất cả bốn cạnh của lưới đều được bao quanh bởi nước.

40. Cấu trúc dữ liệu heap là gì?
## 1.3. Giải thuật
1. Làm thế nào để so sánh hai thuật toán trên cùng một vấn đề?

2. Trường hợp tốt nhất, xấu nhất và trung bình?

3. Bạn hiểu gì về ký hiệu tiệm cận?

4. Viết chương trình đổi giá trị hai số mà không dùng biến tạm?

5. Giải thích về mô hình Chia Để Trị. Liệt kê các thuật toán sử dụng mô hình này?

6. Giải thích về thuật toán tham lam và liệt kê các ví dụ sử dụng nó?

7. Bạn hiểu gì về thuật toán tìm kiếm? Liệt kê vài thuật toán tìm kiếm?

8. Mô tả thuật toán tìm kiếm tuyến tính?

9. Mô tả thuật toán tìm kiếm nhị phân?

10. Viết ra một thuật toán để thêm một nút vào danh sách liên kết được sắp xếp theo thứ tự tăng dần (duy trì thuộc tính sắp xếp)?

11. Viết thuật toán đếm số nút lá trong cây nhị phân?

12. Quy hoạch động là gì? Liệt kê các vấn đề sử dụng quy hoạch động?

13. Viết thuật toán đảo chuỗi. Ví dụ như nhập vào Hello đảo thành Olleh?

14. Bạn hiểu gì về thuật toán BFS (Breadth First Search)?

15. Bạn hiểu gì về thuật toán DFS (Depth First Search)?

16. Cách mà thuật toán mã hoá hoạt động?

17. Các thuật toán mã hoá phổ biến hiện nay?

18. Mô tả thuật toán merge sort?

19. Mô tả thuật toán quick sort?

20. Mô tả thuật toán bubble cùng với ví dụ?

21. Viết thuật toán tính tổng mảng con lớn nhất của một mảng cho trước. Nói cách khác, tìm tổng lớn nhất có thể đạt được bằng cách lấy các phần tử được sắp xếp từ một mảng số nguyên cho trước?

22. Giải thích Thuật toán Dijkstra để tìm đường đi ngắn nhất giữa một nút đến bất kỳ nút nào khác trong đồ thị?

23. Có thể sử dụng tìm kiếm nhị phân với danh sách liên kết không?

24. Thuật toán đệ quy là gì? Nêu các quy tắc quan trọng mà mọi thuật toán đệ quy phải tuân theo?

25. Viết một thuật toán để chèn một nút trong Cây tìm kiếm nhị phân?

26. Phân biệt select sort và insert sort?

27. Định nghĩa duyệt cây và liệt kê một số thuật toán để duyệt cây nhị phân?

28. Mô tả thuật toán sắp xếp Heap sort?

29. Độ phức tạp không gian của thuật toán sắp xếp chèn là gì?

30. Độ phức tạp không gian của thuật toán sắp xếp lựa chọn là gì?
# 2. Front end
## 2.1. HTML
1. Thẻ HTML và phần tử là một?

2. Thẻ và thuộc tính trong HTML là gì?

3. Phần tử void trong HTML là gì?

4. Ưu điểm của khoảng trắng?

5. HTML Entities là gì?

6. Các kiểu danh sách trong HTML?

7. Thuộc tính class trong HTML là gì?

8. Sự khác nhau của thuộc tính id và class trong phần HTML?

9. multipart/form-data là gì?

10. Mô tả cấu trúc của HTML?

11. Cách để tối ưu tải tài nguyên website?

12. Thẻ định dạng khác nhau trong HTML

13. Các thể loại Doctypes khả dụng

14. Hãy giải thích cách hiển thị tập ký tự đang được sử dụng trong HTML?

15. Sự khác nhau giữa thẻ <strong> và <b>, thẻ <em> và <i>?

16. Đặc trưng của thẻ <head> và <body> trong HTML?

17. Chúng ta có thể hiển thị một trang bên trong một trang web hoặc lồng các trang web vào nhau có được không?

18. Cell Padding khác với Cell Spacing như thế nào?

19. Làm thế nào chúng ta có thể ghép hai hoặc nhiều hàng hoặc nhiều cột thành một hàng hoặc cột duy nhất trong một bảng HTML?

20. Có thể thay đổi một phần tử inline thành một phần tử block không?

21. Chúng ta có thể định vị một phần tử HTML bằng bao nhiêu cách?

22. Có bao nhiêu cách để hiển thị phần tử HTML?

23. Sự khác biệt giữa dislay: none và visibility: hidden?

24. Làm sao để chỉ định liên kết trong HTML và giải thích thuộc tính target?

25. Có bao nhiêu cách để dùng CSS cùng với HTML?

26. Sự khác biệt giữa thẻ <link> và thẻ <a>?

27. Cách để thêm code javascript vào HTML?

28. Thẻ script dùng ở head hay body?

29. Form là gì và cách tạo form trong HTML?

30. Xử lý sự kiện trong HTML?

31. Các ưu điểm của HTML5 so với các phiên bản trước?

32. Cách để thêm nhạc và video vào web?

33. Phần tử inline và block trong HTML5?

34. Sự khác biệt giữa thẻ img và thẻ figure?

35. Làm thế nào để chỉ định metadata trong HTML5?

36. Thẻ <datalist> và thẻ <select> là giống nhau?

37. Image Map là gì?

38. Phần tử Sematic là gì?

39. Sự khác biệt giữa thẻ <meter> và thẻ <process>?

40. Có thể kéo và thả trong HTML5 không?

41. Sự khác biệt giữa phần tử SVG và Canvas trong HTML5?

42. Các kiểu file audio có thể dùng trong HTML5?

43. Thông số kỹ thuật trong HTML5 là gì?

44. Các khái niệm về web storage trong HTML5?

45. Microdata là gì?

46. Thẻ nào được dùng để biểu diễn kết quả tính toán?

47. Mối quan hệ giữa thẻ <h1> và <header> trong HTML5?

48. Giải thích về đồ hoạ trong HTML5?

49. Các kiểu input mới trong HTML5 trong form?

50. Các thẻ mới cho phần tử media trong HTML5?

51. Tại sao bạn nghĩ rằng việc bổ sung chức năng kéo và thả trong HTML5 là quan trọng? Bạn sẽ làm thế nào để tạo một hình ảnh có thể kéo được trong HTML5?

52. Tại sao ta cần phần tử MathML trong HTML5?

53. Server-sent events trong HTML5 là gì?

54. Web workers là gì?

55. Việc sử dụng thuộc tính novalidate cho thẻ form trong HTML5 để làm gì?

56. Vector và Raster trong ảnh là gì?

57. Làm thế nào để hỗ trợ SVG trên trình duyệt cũ?

59. File manifest trong HTML5 là gì?

60. API định vị địa lý trong HTML5?

61. Viết code minh hoạ API định vị địa lý?

62. Giải thích về Web Component?
## 2.2. CSS
1. Mô hình hộp trong CSS là gì? Thuộc tính CSS nào là một phần của nó?

2. Ưu điểm của CSS?

3. Hạn chế của CSS?

4. Làm thế nào để đưa CSS vào trang web?

5. Các loại selector khác nhau trong CSS?

6. CSS Preprocessor là gì? SASS vs LESS vs Stylus?

7. VH/VM trong CSS là gì?

8. Sự khác biệt giữa reset và normalize CSS?

9. Sự khác biệt giữa inline, block và inline-block?

10. Việc kiểm tra trang web trên các trình duyệt khác nhau có quan trọng không?

11. Phần tử Pseudo và các lớp Pseudo là gì?

12. Làm thế nào để chỉ định các đơn vị trong CSS? Những cách khác nhau để làm điều đó là gì?

13. Liệu margin-top hoặc margin-bottom có ảnh hưởng đến các phần tử inline không?

14. Thuộc tính nào được sử dụng để thay đổi dạng phông chữ?

15. Sự khác biệt giữa adaptive design và responsive design?

16. Làm thế nào để các CSS Selector ứng với các phần tử của trình duyệt?

17. Border-box khác với content-box như thế nào?

18. Opacity được quy định như thế nào trong CSS3?

19. Tại sao chúng ta nên sử dụng thuộc tính float trong CSS?

20. z-index là gì? Nó hoạt động như thế nào?

21. Các CSS selector sau đây có nghĩa là gì?

22. Thuộc tính flexbox là gì?

23. Xếp tầng trong CSS là gì?

24. Giải thích thuộc tính position CSS?

25. Khi nào thì xảy ra DOM reflow?

26. Sự khác biệt giữa các thuộc tính Box Sizing?

27. Làm sao để căn giữa một div trong một div khác?

28. Kể tên bốn loại thuộc tính @media?

29. Hệ thống grid là gì?

30. Các cách để ẩn phần tử với CSS?

31. :root pseudo-class đề cập tới cái gì?

32. Chiến lược mobile-first là gì?

33. Làm thế nào để khôi phục giá trị mặc định của thuộc tính?

34. Sự khác biệt giữa CSS grid và flexbox?

35. Calc dùng để làm gì?

36. Tuỳ chỉnh thuộc tính biến trong CSS?

37. Sự khác biệt của biến CSS và biến preprocessor (SASS, LESS, Stylus)?

38. * { box-sizing: border-box } là gì?

39. !important trong CSS là gì?

40. Tính đặc hiệu trong CSS?

41. Progressive rendering là gì?

42. Lợi thế của dùng translate() thay vì position absolute?

43. Style nào sẽ được tải và áp dụng trước trong code sau?

44. Làm sao để biết trình duyệt có hỗ trợ một tính năng nhất định không?

45. Cách position absolute hoạt động?

46. Các thuộc tính của overflow trong CSS?

47. Bạn có thể căn chỉnh nội dung của thẻ <p> nằm ngay trung tâm của thẻ <div>?

48. Margin khác Padding như thế nào?

49. Cách để tự động đánh số giá trị heading của section và category?

50. Sự khác biệt giữa nth-child() và nth-of-type()?

51. CSS Sprite là gì?

52. Tweening trong CSS?

53. DOM là gì và cách nó liên kết với CSS?

54. Chi tiết cách CSS hoạt động?
## 2.3. Javascript
1. Sự khác biệt giữa các kiểu dữ liệu trong JavaScript?

2. Giải thích về Hoisting trong Javascript?

3. Sự khác biệt giữa toán tử "==" và "===" là như thế nào?

4. Giải thích về ép kiểu ngầm trong JavaScript?

5. JavaScript là ngôn ngữ kiểu tĩnh hay kiểu động?

6. Thuộc tính NaN trong JavaScript là gì?

7. Giải thích về phép gán quá giá trị và phép gán qua tham chiếu?

8. IIFE là gì trong JavaScript?

9. Giải thích về HOC(Higher Order Functions) trong JavaScript?

10. Giải thích về "this"?

11. Giải thích về phương thức call(), aplly() và bind()?

12. Currying trong JavaScript là gì?

13. Giải thích về Scope và Scope Chain?

14. Giải thích về Closures trong JavaScript?

15. Prototype là gì?

16. Callback là gì?

17. Memoization là gì?

18. Đệ quy là gì?

19. Hàm khởi tạo trong JavaScript

20. DOM là gì?

21. Hàm arrow là gì?

22. Sự khác biết của biến dùng var, let và const

23. Tham số còn lại và toán tử spread là gì?

24 Sử dụng Promise trong JavaScript

25. Lớp trong JavaScript?

26. Hàm generator là gì?

27. Giải thích WeakSet trong JavaScript

28. Giải thích WeakMap trong JavaScript

29. Cấu trúc đối tượng là gì?

30. Temporal Dead Zone là gì?

31. Arrow function khác gì so với regular function về từ khóa this?

32. Phân biệt cơ chế xử lý bất đồng bộ: Callback, Promise và Async/Await.

33. Ứng dụng thực tế của Closure trong JavaScript?
## 2.4. ES6+
1. Các tính năng nổi bật của ES6?

2. Hỗ trợ hướng đối tượng trong ES6 như thế nào?

3. Sự khác biệt giữa ES5 và ES6?

4. Sự khác biệt giữa let và const?

5. Thảo luận về hàm arrow?

6. Khi nào không nên dùng hàm arrow?

7. Hàm generator là gì?

8. Toán tử spread trong ES6?

9. Giải thích về destructuring trong ES6?

10. Promise trong ES6 là gì?

11. Giải thích về Rest parameter trong ES6?

12. Thảo luận về template literal trong ES6?

13. Ưu điểm của class trong ES6?

14. Cách tạo class trong ES6

15. Biểu thức lớp là gì?

16. Tham số mặc định là gì?

17. Bạn hiểu gì về IIFE?

18. Các trạng thái của Promise trong ES6?

19. Export Default và Name Export trong ES6?

20. Từ khoá nào dùng cho kế thừa trong ES6?

21. Bubbling và Capturing là gì?

22. Sự khác biệt giữa for...of và for..in?

23. Symbol trong ES6 là gì?

24. Babel là gì?

25. Tên các phương thức mảng có trong ES6

26. Tên các hàm chuỗi có trong ES6?

27. So sánh code ES5 và ES6 cho tạo và tách đối tượng?

28. Hoán vị với Destrucuring Assignment?

29. Kết quả của toán tử spread ở code bên dưới?

30. Prototype Design Pattern với ES6?

31. WeakMap trong ES6 là gì? Nó khác Map như thế nào?

32. Lợi thế của hàm khởi tạo với cú pháp arrow?

33. Temporal Dead Zone là gì?

34. Sự khác biệt giữa Set và WeakSet trong ES6?

35. Proxy trong ES6 là gì?

36. Sự khác biệt giữa const và Object.freeze()?

37. Tại sao đoạn code này không hoạt động như một IIFE? Điều gì cần được sửa đổi để nó được phân loại là IIFE?

38. Giải thích về Internationalization và Localization?

39. Webpack là gì?
## 2.5. Typescript
1. Các kiểu nguyên thuỷ trong TypeScript?

2. Giải thích cách mảng làm việc trong TypeScript?

3. Kiểu any là gì?

4. Kiểu void là gì?

5. Kiểu unknown là gì?

6. Các cách khai báo biến trong TypeScript?

7. Cung cấp cú pháp cho hàm với chú thích kiểu?

8. Cách tạo đối tượng trong TypeScript?

9. Tuỳ chọn thuộc tính trong TypeScript?

10. Giải thích khái niệm null trong TypeScript?

11. undefined trong TypeScript là gì?

12. Giải thích kiểu never trong TypeScript?

13. Cách enum hoạt động trong TypeScript?

14. Toán tử typeof trong TypeScript là gì?

15. Tham số còn lại trong TypeScript?

16. Tham số destructuring là gì?

17. Giải thích về cú pháp class trong TypeScript?

18. Giải thích cú pháp arrow function trong TypeScript?

19. Tham số tuỳ chọn trong TypeScript?

20. Mục đích của file tsconfig.json?

21. Liệt kê các kiểu vòng lặp trong TypeScript?

22. Giải thích về kiểu symbol trong TypeScript?

23. Chuỗi tuỳ chọn trong TypeScript hoạt động thế nào?

24. Nạp chồng hàm trong TypeScript?

25. Tự suy kiểu là gì?

26. Contextual Typing là gì?

27. noImplicitAny là gì?

28. Interface là gì?

29. Các quyền truy cập trong TypeScript?

30. TypeScript có hỗ trợ static class không?

31. Lớp trừu tượng là gì?

32. Hàm ẩn danh là gì?

33. Kiểu union trong TypeScript?

34. Kiểu intersection là gì?

35. Alias là gì?

36. Kiểu tuple trong TypeScript?

37. Giải thích cách hoạt động tuple destructuring trong TypeScript?

38. Xác nhận kiểu trong TypeScript là gì?

39. Cách để ép kiểm tra null trong TypeScript?

40. Cách làm cho thuộc tính đối tượng bất biến trong TypeScript?

41. File khai báo kiểu là gì?

42. Các chỉ thị ba dấu gạch chéo là gì?

43. Giải thích toán tử in?

44. Giải thích từ khoá implement trong TypeScript?

45. Các kiểu chuỗi ký tự là gì?

46. Các template literal là gì?

47. Kế thừa trong TypeScript?

48. Kiểu điều kiện là gì?

49. Kiểu hàm trong TypeScript?

50. Liệt kê một số kiểu tiện ích được cung cấp bởi TypeScript và giải thích cách sử dụng của chúng?
## 2.6. Web API
1. Tại sao Web API quan trọng?

2. Chính xác thì Web API là gì?

3. Kiểu trả về được hỗ trợ trong Web API?

4. Sự khác biệt giữa WCF và Web API?

5. Web API tốt hơn WCF ở điểm nào?

6. Sự khác biệt giữa REST API và RESTful API?

7. Ưu điểm của sử dụng REST trong Web API?

8. Sự khác biệt giữa REST và SOAP?

9. Giao thức hỗ trợ Web API?

10. XML và JSON là gì?

11. Đối tượng sử dụng Web API?

12. Web API và MVC có gì khác?

13. CORS là gì?

14. JWT (JSON Web Token) hoạt động như thế nào trong hệ thống fullstack?
## 2.7. React JS
1. React là gì

2. Lợi ích khi dùng React?

3. Hạn chế của React?

4. useState() trong React là gì?

5. Keys trong React?

6. JSX là gì?

7. Sự khác biệt giữa class component và function component?

8. Virtual DOM là gì?

9. Sự khác biệt giữa controlled component và uncontrolled component?

10. Props trong React là gì?

11. Giải thích state và props?

12. Các kiểu side effect trong React component?

13. Prop drilling trong React?

14. Error boundary là gì?

15. React Hooks là gì?

16. Tại sao lại cần React Hook?

17. Các quy tắc sử dụng React Hooks?

18. Cách dùng useEffect?

19. Sử dụng refs với React Hooks?

20. Hook tuỳ chỉnh là gì?

21. Strict mode trong React là gì?

22. Ngăn chặn re-render trong React?

23. Các cách khác nhau để chỉnh style component?

24. Các kỹ thuật tối ưu hiệu suất ứng dụng React?

25. Truyền dữ liệu giữa các component?

26. High-Order Component là gì?

27. Các giai đoạn trong vòng đời component?

28. Các phương thức trong vòng đời component?

29. React hook có làm việc với static typing?

30. Các kiểu Hooks trong React?

31. Sự khác biệt giữa lớp và React Hook?

32. Hiệu suất của React Hook so với lớp?

33. Các hook có thay thế được lớp hoàn toàn?

34. React Router là gì?

35. React Hook có thể thay thế Redux?

36. Render có điều kiện trong React?

37. Sự khác biệt giữa NavLink và Link?

38. withRouter trong react-router-dom là gì?

39. Cách hiển thị dữ liệu API với Axios?

40. Caching trong React?

41. Tại sao không nên cập nhật state trực tiếp mà phải dùng setState hoặc hàm cập nhật của useState?

42. Virtual DOM trong React hoạt động như thế nào (diffing và reconciliation)?

43. Redux Thunk và Redux Saga khác nhau cơ bản ở điểm nào?

44. Khi nào nên tách logic thành Custom Hook trong React?

45. Trong JSX, thuộc tính className khác thuộc tính class trong HTML như thế nào?

46. Dependency array trong useEffect có ý nghĩa gì?

47. Cleanup function trong useEffect dùng trong trường hợp nào?
## 2.8. Git
1. Trong Git Flow, nhánh develop và master (main) khác nhau thế nào?

2. Lệnh git merge và git rebase khác nhau điểm nào cơ bản?

3. Lệnh git checkout dùng để làm gì?

4. Lệnh git stash dùng để làm gì?

5. Sự khác biệt giữa git pull và git fetch?

6. Cách xử lý merge conflict khi làm việc nhóm lớn?
## 2.9. Agile / Scrum
1. Ba cột cơ bản trong bảng Kanban hoặc board Scrum thường gặp là gì?

2. Scrum Master đóng vai trò gì trong nhóm?

3. Sprint Backlog là gì?

4. Trong Agile, nếu một task không hoàn thành khi kết thúc sprint thì xử lý thế nào?

5. Product Owner trong Scrum chịu trách nhiệm chính về điều gì?
# 3. Backend
Chọn Java hoặc C# nhé.
## 3.1. Java
1. Java khác với C/C++ như thế nào?

2. JVM là gì?

3. Bộ dọn rác trong Java?

4. Các kiểu / đặc điểm lớp trong Java?

5. Sự khác biệt giữa lớp trừu tượng và interface?

6. Java có ref / out như C# không? Thay thế ra sao?

7. “Mở rộng” phương thức trong Java?

8. Generic trong Java?

9. Java có lớp partial như C# không?

10. Late binding và early binding trong Java?

11. Mảng trong Java là gì?

12. Sự khác biệt giữa mảng (array) và ArrayList?

13. Tính đa hình (Polymorphism) trong Java thể hiện qua những cách nào?

14. Phân biệt ArrayList và LinkedList trong Java Collections.

15. Phân biệt interface Map và class HashMap trong Java.

16. Tại sao việc sử dụng Generics trong Java lại quan trọng?

17. Làm thế nào để tránh lỗi ConcurrentModificationException khi duyệt một Collection trong Java?

18. JDBC trong Java là gì?

19. PreparedStatement khác Statement trong JDBC thế nào?

20. Phân biệt HashMap và TreeMap trong Java.

21. Phân biệt HashSet và TreeSet trong Java.
## 3.1.1. Spring Boot
1. Dependency Injection (DI) là gì?

2. Sự khác biệt giữa @Controller và @RestController trong Spring?

3. Transaction trong Spring Boot: sự khác biệt giữa @Transactional ở mức class và mức method?

4. Giải thích cơ chế xử lý exception tập trung trong Spring Boot (@ControllerAdvice, @ExceptionHandler).

5. So sánh Hibernate (JPA) và JDBC thuần. Khi nào nên dùng cái nào?

6. N+1 Query Problem là gì và cách khắc phục trong JPA/Hibernate?

7. Làm thế nào để bảo mật API trước tấn công SQL Injection trong hệ thống Java?
## 3.2. C#
1. C# khác với C/C++ như thế nào?

2. CLR là gì?

3. Bộ dọn rác trong C#?

4. Các kiểu class trong C#?

5. Sự khác biệt giữa lớp trừu tượng và interface?

6. Sự khác biệt giữa từ khoá ref và out?

7. Phương thức mở rộng trong C#?

8. Generic trong C#?

9. Lớp partial trong C# là gì?

10. Sự khác biệt giữa late binding và early binding trong C#?

11. Mảng trong C# là gì?

12. Sự khác biệt giữa Array và ArrayList trong C#?

13. Kế thừa và đa kế thừa trong C#?

14. Boxing và Unboxing trong C#?

15. Thuộc tính trong C# là gì?

16. Indexer trong C# là gì?

17. Sự khác biệt giữa toán tử == và phương thức equals() trong C#?

18. Nạp chồng trong C#

19. Reflection trong C#?

20. Sự khác biệt giữa hằng và readonly trong C#?

21. Sự khác biệt giữa String và StringBuilder?
## 3.3. web service
1. Giải thích các kiểu dịch vụ web khác nhau?

2. Các tính năng quan trọng của dịch vụ Web?

3. Các thành phần của dịch vụ Web?

4. Khác biệt giữa API và dịch vụ Web?

5. Các công cụ kiểm thử dịch vụ web?

6. WSDL là gì?

7. XML-RPC là gì?

8. Tính năng của XML-RPC?

9. UDDI là gì?

10. Các tính năng nổi bật của UDDI

11. Tên ngôn ngữ thường được sử dụng bởi UDDI?

12. Giải thích về kiến trúc dịch vụ web?

13. Web Service Provider là gì?

14. Web Service Requestor là gì?

15. Web Service Registry là gì?

16. Các lớp khác nhau của ngăn xếp giao thức dịch vụ web?

17. Giải thích thuật ngữ Synchronicity?

18. Dịch vụ web RESTful là gì?

19. Ưu điểm của dịch vụ Web RESTful?

20. Giao thức dùng cho dịch vụ Web RESTful?

21. Giải thích stateless trong dịch vụ web RESTful?

22. Các phương thức HTTP cho dịch vụ web RESTful?

23. Các Status code trong REST API?

24. SOAP là gì?

25. Các phần tử của SOAP message?

26. SOA là gì?

27. Ưu điểm của SOA?

28. Các vấn đề bảo mật trong dịch vụ Web?

29. Các thành phần cần được công khai khi triển khai dịch vụ web?

30. Tại sao dịch vụ web lại quan trọng?

31. DISCO là gì?

32. Giải thích về BEEP?

33. URL trong dịch vụ web REST?

34. Bảo mật trong dịch vụ web?

35. Sự khác biệt của dịch vụ web với CORBA và DCOM?

36. Phân biệt phương thức HTTP PUT và PATCH trong RESTful API?
## 3.4. Nginx
### 1. Nginx hoạt động như thế nào?

### 2. Các tính năng của Nginx Server?

3. Sự khác biệt giữa Nginx và Apache?

4. Nginx xử lý yêu cầu HTTP như thế nào?

5. Trong Nginx, làm cách nào để ngăn chặn các yêu cầu xử lý với tên server không xác định?

6. Ưu điểm của "reverse proxy server"?

7. Cách dùng Nginx tốt nhất?

8. Tiến trình master và worker trong Nginx?

9. Giải thích cách khởi động Nginx ở cổng khác 80?

10. Có thể thay thế lỗi 502 thành 503 trong Nginx không?

11. Trong Nginx, làm thế nào giữ dấu gạch chéo trong URLs?

12. ngx_http_upstream_module là gì?

13. Vấn đề C10K là gì?

14. Cách dùng stub_status và sub_filter directives?

15. Giải thích Nginx có hỗ trợ nén yêu cầu lên upstream không?

16. Cách lấy thời gian hiện tại trong Nginx?

17. Giải thích -s trong Nginx server?

18. Cách thêm module vào Nginx server?
## 3.5. Websocket
1. WebSockets là gì?

2. Giải thích Server-Sent Events/EventSource?

3. Tương tác với độ trễ thấp là gì?

4. Short Polling là gì?

5. Tại sao dùng WebSocket thay vì HTTP?

6. Giải thích Long Polling là gì?

7. Lợi ích của SSE so với WebSocket?

8. Giải thích các kỹ thuật giao tiếp khác nhau trên web mà bạn biết?

9. Sự khác biệt giữa SSE và WebSocket?

10. Giải thích về giao thức WebSocket?

11. WebSocket Frame là gì?

12. Khi nào WebRTC tốt hơn WebSocket?
## 3.6. Docker & Kiến trúc hệ thống
1. Docker Image và Docker Container khác nhau thế nào?

2. Dockerfile dùng để làm gì?

3. docker-compose.yml dùng để làm gì?

4. Volume trong Docker là gì và khi nào cần dùng?

5. So sánh kiến trúc Monolith và Microservices.

6. Ưu và nhược điểm chính của Microservices so với Monolith?

7. API Gateway trong kiến trúc Microservices đóng vai trò gì?

8. Service Discovery trong Microservices là gì?

9. Eventual consistency trong hệ phân tán là gì?

10. Horizontal scaling khác vertical scaling thế nào?
# 4. Database
# 4.1. SQL
1. Cơ sở dữ liệu là gì?

2. DBMS là gì?

3. RDBMS là gì? Nó khác với DBMS như thế nào

4. SQL là gì?

5. Sự khác biệt giữa SQL và MySQL?

6. Bảng và trường là gì?

7. Ràng buộc trong SQL?

8. Khoá chính là gì?

9. Ràng buộc UNIQUE là gì?

10. Khoá ngoại là gì?

11. JOIN là gì? Liệt kê các kiểu JOIN?

12. Self-Join là gì?

13. Cross-Join là gì?

14. Đánh chỉ mục là gì? Giải thích các kiểu chỉ mục khác nhau?

15. Toàn vẹn dữ liệu là gì?

16. Truy vấn là gì?

17. Truy vấn con là gì? Các kiểu truy vấn con?

18. Lệnh SELECT là gì?

19. Các mệnh đề phổ biến được dùng với SELECT trong SQL?

20. Các lệnh UNION, MINUS và INTERSECT là gì?

21. Con trỏ là gì? Cách dùng con trỏ?

22. Thực thể và quan hệ là gì?

23. Các kiểu quan hệ trong SQL?

24. Alias trong SQL là gì?

25. View là gì?

26. Normalization là gì?

27. Denormalization là gì?

28. Các dạng chuẩn hoá?

29. Lệnh TRUNCATE, DELETe và DROP là gì?

30. Sự khác biệt giữa lệnh DELETE và TRUNCATE là gì?

31. Sự khác biệt giữa lệnh DROP và TRUNCATE là gì?

32. Các hàm Aggregate và Scalar là gì?

33. Hàm người dùng định nghĩa là gì?

34. OLTP là gì?

35. Sự khác biệt giữa OLTP và OLAP?

36. Collation là gì?

37. Stored Procedure là gì?

38. Stored Procedure đệ quy là gì?

39. Làm thế nào để tạo một bảng trống có cấu trúc giống hệt một bảng khác?

40. Pattern Matching trong SQL?

41. Sự khác biệt giữa clustered index và non-clustered index?

42. Khi nào không nên tạo index trên một cột?
# 4.2. MySQL
1. MySQL là gì?

2. Ưu điểm của MySQL?

3. Bạn hiểu "cơ sở dữ liệu" là gì?

4. Chuẩn SQL là gì?

5. Một cơ sở dữ liệu MySQL bao gồm những gì?

6. Làm thế nào để tương tác với MySQL?

7. Truy vấn MySQL là gì?

8. Các lệnh phổ biến trong MySQL?

9. Cách tạo cơ sở dữ liệu trong MySQL?

10. Cách tạo bảng trong MySQL?

11. Cách chèn dữ liệu vào MySQL?

12. Xoá cột trong MySQL?

13. Đánh chỉ mục trong MySQL?

14. Xoá dữ liệu trong bảng MySQL?

15. Cách để xem cơ sở dữ liệu trong MySQL?

16. Các kiểu số trong MySQL?

17. Các kiểu chuỗi trong MySQL?

18. Kiểu dữ liệu thời gian là gì?

19. BLOB trong MySQL là gì?

20. Cách thêm người dùng vào MySQL?

21. View trong MySQL là gì?

22. Tạo và thực thi view trong MySQL?

23. MySQL Trigger là gì?

24. Có bao nhiêu Trigger khả dụng trong MySQL?

25. MySQL Server là gì?

26. MySQL Client là gì?

27. Các kiểu quan hệ trong MySQL?

28. Giải thích kiến trúc của MySQL?

29. Mở rộng trong MySQL?

30. Sharding trong MySQL?

31. Transaction Storage Engines trong MySQL là gì?