# Khung nội dung Slide: BÁO CÁO TỔNG KẾT THỰC TẬP TẠI IOC 
*(Dành cho Sinh viên (Thực tập sinh) trình bày tại buổi bảo vệ cuối kỳ trước Mentor/Hội đồng)*

---

## Nội dung 1: Giới thiệu (Cover Slide)
*   **Tiêu đề:** Báo cáo Tổng kết kỳ Thực tập tại Internship OneConnect (IOC)
*   **Tên dự án thực tập:** [Tên Hệ thống Fullstack đã làm - VD: Hệ thống Quản lý Bán hàng / Nền tảng Học trực tuyến]
*   **Thành viên / Nhóm trình bày:** [Họ và tên sinh viên hoặc Danh sách nhóm] - [Trường/Lớp]
*   **Vị trí thực tập:** Fullstack Developer
*   **Thời gian thực tập:** [Tháng/Năm] - [Tháng/Năm]

---

## Nội dung 2: Mục tiêu & Phạm vi Dự án (Project Scope)
*   **Mô tả ngắn gọn dự án:** Hệ thống này làm gì? Giải quyết vấn đề gì?
*   **Phạm vi dự án & Vai trò:** Nêu rõ đây là dự án **Cá nhân (Solo Project)** hay **Nhóm (Group Project)**.
    *   *Nếu là cá nhân:* Làm việc độc lập 100% từ phân tích, thiết kế, đến lập trình Backend/Frontend.
    *   *Nếu là nhóm:* Phân rõ vai trò (Ai làm DB, Backend, Frontend) và cách team quản lý tiến độ bằng áp dụng linh hoạt các mô hình Agile/Scrum, Kanban, Waterfall.

---

## Nội dung 3: Thiết kế Hệ thống & Phân tích Dữ liệu (System Design & DB)
*   **Biểu đồ Use case**
*   **Mô hình Thực thể Kết hợp (ERD):** Hiển thị sơ đồ các bảng CSDL trọng tâm.
*   **Sơ đồ Kiến trúc (Architecture Diagram)**
*   **Công nghệ sử dụng**

---

## Nội dung 4: Quản lý Dự án theo các mô hình Agile/Scrum, Kanban, Waterfall (Project Management)
*   **Product Backlog & User Stories:** Khái quát danh sách các tính năng cốt lõi đã định nghĩa cho hệ thống.
*   **Quá trình chạy Sprint:**
    *   Tổng số Sprint đã thực hiện, mục tiêu (Sprint Goal) của từng Sprint.

---

## Nội dung 5: Demo sản phẩm và trả lời các câu hỏi trước hội đồng 
*   **Demo tính năng**
*   **Show source code:** trình bày rõ ràng về cấu trúc source code
*   **(Optional) Báo cáo về bảo mật (nếu nhóm có)**
*   **Q&A**

---

## Nội dung 6: Kết luận & Lời cảm ơn (Conclusion)
*   **Kết luận về sản phẩm** 
*   **Lời cảm ơn**